                ...
                >
                  ...
                  <template v-if="['checkbox', 'radio'].indexOf(funcMode) !== -1">
                    <view
                      ...
                      v-if="selectParent === true ? true : item.lastRank"
                    >
                      <view
                  ...
                  <template v-else-if="funcMode === 'edit'">
                    ...
                    <view v-else>
                      ...
                      <template v-else>
                        ...
                        >
                        <text
                          @click="_addSubItem(item)"
                          :style="{ color: item.disabled ? '#ccc' : '#333' }"
                          class="iconfont icon-btn"
                          >&#xe664;</text
                        >
                        <text
                          @click="_addSameItem(item)"
                          :style="{ color: item.disabled ? '#ccc' : '#333' }"
                          class="iconfont icon-btn"
                          >&#xe601;</text
                        >
                      ...
                      <text
                        @click="_delItem(item)"
                        :style="{ color: item.disabled ? '#ccc' : '#ff4d4f' }"
                        class="iconfont icon-btn"
                        >&#xe602;</text
                      >
                    </view>
...
<script>
import nextSearchMore from "../next-search-more/next-search-more.vue";
export default {
  name: "next-tree",
  ...
  methods: {
    ...
    getRenderTreeList(list = [], rank = 0, parentId = [], parents = []) {
      ...
      list.forEach(item => {
        ...
        treeList.push({
          ...
          status: "",
          operateCallback:
            typeof item.operateCallback === "function" ? item.operateCallback : undefined
        });
        if (bool1) {
    ...
    async _treeItemTap(item, _index) {
      ...
      if (item.lastRank === true) {
        ...
        if (this.changeVerify && typeof this.changeVerify === "function") {
          ...
          const tip = this.changeVerify(
            current,
            this.multiple ? this._getCheckedParams() : [current]
          );
          if (tip) {
            this.treeList[index].checked = !this.treeList[index].checked;
            uni.showToast({
      ...
      if (isLoadData) {
        ...
        const lists = this.getRenderTreeList(
          newChild || [],
          item.rank + 1,
          parentId.concat([item.id]),
          [{ [this.valueKey]: item[this.valueKey], [this.labelKey]: item[this.labelKey] }]
        );
          ...
          if (childItem.parentId.includes(id) && !this.foldAll) {
            ...
            if (childItem.id !== id) {
              const bool1 = item.parentId.some(id => {
                return (
                  (childItem.parentId && childItem.parentId.indexOf(id) !== -1) ||
                  childItem.rank === 0
                );
              });
              ...
              const nodeList = [
                item,
                { parentId: childItemParentId.length ? childItemParentId : [] }
              ];
              if (!childItem.lastRank) {
                ...
                if (!bool1 && !this.isSiblingNode([item, childItem])) {
                  ...
                  childItem.rank !== 0 &&
                  _id !== id &&
                  !this.isSiblingNode(nodeList) &&
                  this.isParentSiblingNode(nodeList)
                ) {
                  childItem.show = false;
                }
              } else {
    ...
    updateParentChecked(index) {
      ...
      if (parentId && parentId.length) {
        parentId.map(id => {
          ...
          const childrenIds = (parentTreeDataItem[this.childrenKey] || []).map(
    ...
    updateHalfChecked(index) {
      ...
      if (this.treeList[index].checked == false) {
        ...
        const checkedKeyList = this.getChildrenKeys(children);
        const bool = this.treeList
          .filter(item => checkedKeyList.indexOf(item.id) !== -1)
